<template>  
    <view class="container">  
		...
		<view class="user-section">
			<image class="bg" :src="baseUrl+'/img/appFile/user-bg.jpg'"></image>
			<view class="user-info-box">
				<view class="portrait-box">
					<image style="background-color: white;" class="portrait" :src="getHead()"></image>
				</view>
				<view class="info-box">
					<text class="username">{{user.nick || '游客'}}</text>
				</view>
			</view>
		</view>
		...
		>
			<image class="arc" :src="baseUrl+'/img/appFile/arc.png'"></image>
			
			<view v-if="user.type == 3">
				<view class="tj-sction u-p-t-20 u-font-30" style="color: #F76260;">
					管理员专区
				</view>
				<view class="">
					<u-tabs :show-bar="false" :bold="false" active-color="#ADADAD" inactive-color="#ADADAD"  :list="list" :is-scroll="false" :current="current" @change="change" ></u-tabs>
				</view>
			</view>
			...
			<!-- 浏览历史 -->
			<view class="history-section icon" v-if="user.type == 1 || user.type == 3">
				<view class="sec-header">
					<text>其他功能</text>
				</view>

				<list-cell icon="icon-shezhi1" iconColor="#e07472" title="设置"  @eventClick="navTo('/pages/set/set')"></list-cell>
				<list-cell icon="icon-shoucang_xuanzhongzhuangtai" iconColor="#54b4ef" @eventClick="navTo('/pages/public/allCollect')" title="我的收藏"></list-cell>
...
<script>  
    ...
    export default {
		...
		onShow() {
			...
			if(this.user){
				this.list = this.user.type == 3 ? [{name:"食谱添加"},{name:"食材添加"},{name:"订单管理"}] : [];
			}
