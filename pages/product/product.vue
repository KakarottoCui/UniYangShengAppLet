...
<script>
	...
	export default{
		...
		onLoad(options){
			...
			this.goodsInfo = appRequest.goodsCache.getCache();
		...
		methods:{
			...
			addGoods:function(){
				...
				if(!this.user){
					this.$api.msg("游客不能使用购物车，请登录");
					return;
				}
				
				let _this = this;
				let info = {
					goodsFk:_this.goodsInfo.id,
					storeFk:_this.goodsInfo.storeFk,
					price:_this.goodsInfo.price,
					userFk:_this.user.userNo,
					num:1,
			...
			buy(item){
				...
				if(this.user.type != 1 && this.user.type != 3 ){
					this.$api.msg("游客不能参与购物，请登录");
					return;
				}
				uni.navigateTo({
					url: `/pages/order/createOrder?data=${JSON.stringify(
						[{id:0,gid:_this.goodsInfo.id,num:1}]
					)}`
				})
			},
			stopPrevent(){}
