					...
					>
						...
						<scroll-view v-if="item.goods.length > 1" class="goods-box" scroll-x>
							<view
								...
								class="goods-item"
							>
							<view class="pic-list u-flex" style="flex-direction: column;">
								<image class="goods-img" :src="picUrl + goodsItem.pic1" mode="aspectFill"></image>
								<text class="price u-font-10 u-tips-color">{{"￥"+goodsItem.price +" x "+ goodsItem.num }}</text>
							</view>
						...
						>
							...
							<view class="right">
								...
								<!-- <text class="attr-box">{{goodsItem.price}}  x {{goodsItem.num}}</text> -->
						...
						<view class="price-box">
							共
							<text class="num">{{getGoodsNum(item.goods)}}</text>
							件商品 实付款
						...
						<view class="action-box b-t" v-if="item.stat == 2 && user.userNo == item.userFk">
							<button class="action-btn recom" @click="changeOrder(item.id,3)">收货</button>
						</view>
						<view class="action-box b-t" v-if="item.stat == 1 && user.type == 3">
...
<script>
	...
	export default {
		...
		onLoad(options){
			...
			this.getOrders(this.tabCurrentIndex == 0?null:this.tabCurrentIndex);
		...
		methods: {
			...
			commentSub(){
				...
				appRequest.request({
					method: "POST",
					url: appRequest.setCommnt,
					data:{
						data:JSON.stringify(this.selItem)
					},
			...
			getComment(item){
				this.selItem = item.goods;
				this.selItem.map(function(obj,index,arr){
					obj.memo = "";
					obj.type = obj.id;
				});
				this.show = true;
						...
						if(res.confirm){
							...
							appRequest.request({
								...
								success: function(res) {
									if(res.data.code == 200){
										_this.$api.msg("操作成功");
										_this.getOrders(_this.tabCurrentIndex == 0?null:_this.tabCurrentIndex);
									}else{
										_this.$api.msg(res.data.msg);
									}
								},
			...
			getOrders(stat){
				...
				appRequest.request({
					...
					success: function(res) {
						if(res.data.code == 200){
							_this.navList[_this.tabCurrentIndex].orderList = res.data.data;
						}else{
							_this.$api.msg(res.data.msg);
						}
					},
